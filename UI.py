...
def get_user_name():
    user_table = {
        ...
        't609321': 'Verona Nexhipi',
        't595527': 'Liri Osmani',
        't657331': 'Salvi De Biassi',
        't627310': 'Cyrine Manaï',
        't559485': 'Jana Jaccotmuuh-Joseph',
        't585500': 'Geraldine Larsson',
        't602332': 'Steven Friedli',
    }
...
class MyWindow(QWidget):
    ...
    def initUI(self):
        ...
        gridLayout.addWidget(self.requestor_label, 0, 2)
        gridLayout.addWidget(self.requestor_txt, 0, 3)

        # Requestor GPN
        self.requestorG_label = QLabel('Requestor GPN:')
        self.requestorG_txt = QLineEdit()
        gridLayout.addWidget(self.requestorG_label, 1, 2)
        gridLayout.addWidget(self.requestorG_txt, 1, 3)

        # No. of Invoices:
        self.invoice_label = QLabel('No of Invoices:')
        self.invoice_txt = QLineEdit()
        gridLayout.addWidget(self.invoice_label, 5, 2)
        gridLayout.addWidget(self.invoice_txt, 5, 3)

        # Handover
        self.handover_label = QLabel('Handover Type:')
        self.handover_txt = QComboBox()
        self.handover_txt.addItems(['None', 'Genesys', 'Other channel'])
        gridLayout.addWidget(self.handover_label, 6, 2)
        gridLayout.addWidget(self.handover_txt, 6, 3)
        # Trip ID
        self.trip_label = QLabel('Case ID:')
        self.trip_txt = QLineEdit()
        gridLayout.addWidget(self.trip_label, 3, 2)
        gridLayout.addWidget(self.trip_txt, 3, 3)
        mainLayout.addLayout(gridLayout)

        # Report Name
        self.report_label = QLabel('Report Name:')
        self.report_txt = QLineEdit()
        gridLayout.addWidget(self.report_label, 4, 2)
        gridLayout.addWidget(self.report_txt, 4, 3)

        # Nr. of iteration
        self.line_label = QLabel('Total entered line items:')
        self.line_txt = QLineEdit()
        gridLayout.addWidget(self.line_label, 6, 2)
        gridLayout.addWidget(self.line_txt, 6, 3)
        ...
        gridLayout.addWidget(self.nr_of_iter_label, 0, 4)
        gridLayout.addWidget(self.nr_of_iter_txt, 0, 5)

        # Reason of pending
        self.reason_label = QLabel('Reason of pending:')
        self.reason_txt = QLineEdit()
        gridLayout.addWidget(self.reason_label, 1, 4)
        gridLayout.addWidget(self.reason_txt, 1, 5)

        # Action
        self.action_label = QLabel('Action:')
        self.action_txt = QLineEdit()
        gridLayout.addWidget(self.action_label, 2, 4)
        gridLayout.addWidget(self.action_txt, 2, 5)
        ...
        gridLayout.addWidget(self.filter_label, 3, 4)
        gridLayout.addWidget(self.filter_button, 3, 5)
        self.filter_button.clicked.connect(self.filter)

        mainLayout.addLayout(gridLayout)
        mainLayout.addWidget(self.idLabel)

        # Table for DB
        self.tableWidget = QTableWidget()
        self.tableWidget.setColumnCount(9)
        col_names = ['ID', 'Start Date', 'Finish Date', 'Event Date', 'Handover Type', 'Requestor CA/QA',
                     'Report name', 'Case Entered', 'Requestor GPN']
        col_names += ['Rank', 'No of Invoices', 'Number of credit card line items', 'No of cash expenses',
...
def initDB(self):
    self.conn = pyodbc.connect(
        r"DRIVER={Microsoft Access Driver (*.mdb, *.accdb)};DBQ=\\ubsprod.msad.ubs.net\groupshares\GLOBAL\..."
    )
...
def createNewCase(self):
    ...
    sql = ('INSERT INTO [Expense_Management] ([Start Date], [Finish Date], [Event Date], [Handover Type], '
           '[Requestor CA/ DH], [Trip ID], [Report name], [Case entered by], [Requester GPN], '
